(() => {
    ...
    if (!map && document.getElementById("map")) {
      ...
      if (typeof L !== "undefined") {
        try {
          ...
          mapEl.innerHTML =
            '<div style="width:100%;height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center;background:linear-gradient(135deg, #667eea 0%, #764ba2 100%);color:white;padding:20px;text-align:center;"><div style="font-size:48px;margin-bottom:10px;">🗺️</div><p style="margin-bottom:5px;font-size:16px;font-weight:600;">Map Loading...</p></div>';
        }
      } else {
  ...
  function renderFlightTable(flights) {
    ...
    let tableHTML =
      '<div class="flight-table-wrapper"><table class="flight-table"><thead><tr><th>Airline</th><th>Departure</th><th>Arrival</th><th>Duration</th><th>Stops</th><th>Price</th></tr></thead><tbody>';
    ...
    flights.slice(0, 5).forEach((flight) => {
      ...
      const logoHTML = flight.airline_logo
        ? `<img src="${flight.airline_logo}" alt="${
            flight.airline || ""
          }" class="airline-logo-small" onerror="this.style.display='none'">`
        : "";
      ...
      tableHTML += `
        <tr>
          ...
          <td>${depParsed.code || "N/A"}<br><small>${
        depParsed.time || ""
      }</small></td>
          <td>${arrParsed.code || "N/A"}<br><small>${
        arrParsed.time || ""
      }</small></td>
  ...
  function hotelCard(h, index) {
    ...
    if (
      h.image_url &&
      h.image_url.trim() &&
      !h.image_url.includes("placeholder")
    ) {
    ...
    content.innerHTML = `
      ...
      ${
        h.rating
          ? `<div class="card-rating">${ratingStars} ${h.rating}/5</div>`
          : ""
      }
      ...
      <a href="${
        h.link || "#"
      }" target="_blank" class="card-link">View Details →</a>
    `;
  ...
  function flightCard(f, index) {
    ...
    routeDiv.innerHTML = `
      ...
      <div class="flight-route-label">${
        f.stops === "Nonstop" ? "Nonstop" : f.stops || "1+ stops"
      }</div>
    `;
    ...
    const bookingLinks = [];
    if (f.booking_link)
      bookingLinks.push({ name: "Google", url: f.booking_link });
    if (f.kayak_link) bookingLinks.push({ name: "Kayak", url: f.kayak_link });
    if (f.skyscanner_link)
      bookingLinks.push({ name: "Skyscanner", url: f.skyscanner_link });
    if (f.expedia_link)
      bookingLinks.push({ name: "Expedia", url: f.expedia_link });
    if (f.booking_com_link)
      bookingLinks.push({ name: "Booking.com", url: f.booking_com_link });
    if (f.momondo_link)
  ...
  async function sendChatMessage(message) {
    ...
    if (
      msgLower.includes("round trip") ||
      msgLower.includes("roundtrip") ||
      (msgLower.includes("return") && msgLower.includes("date"))
    ) {
      context.roundTrip = true;
    } else if (
      msgLower.includes("one way") ||
      msgLower.includes("one-way") ||
    ...
    try {
          ...
          if (text) {
            ...
            const isDuplicateQuestion =
              ...
              (text === "When would you like to travel?" ||
                text ===
                  "When would you like to travel? When would you like to travel?" ||
                (text.includes("What dates would you like") &&
                  context.departDate &&
                  context.returnDate) ||
                (text.includes("What are your travel interests") &&
                  context.destination));
            ...
            if (!isDuplicateQuestion) {
              ...
              if (flightsForChat.length > 0) {
                ...
                const filteredLines = lines.filter((line) => {
                  ...
                  if (
                    /^\d+\.\s*[A-Za-z\s]+\s*\|\s*Flight\s+\d+/i.test(trimmed)
                  ) {
                    foundFlightList = true;
                    return false;
                  }
              ...
              addMsgWithVoice(
                filteredText,
                "bot",
                flightsForChat.length > 0 ? flightsForChat : null
              );
            }
          }
      ...
      const msgLower = message.toLowerCase();
      hasHotelRequest =
        /hotel|accommodation|stay|lodging|find hotels|show hotels|search hotels/.test(
          msgLower
        );
      hasPlanRequest =
        /travel plan|plan|places to visit|things to do|activities|itinerary/.test(
          msgLower
        );
      hasFlightRequest =
        /flight|flights|find flights|show flights|search flights/.test(
          msgLower
        );
      hasSearchRequest =
        /restaurant|restaurants|cafe|cafes|best|find|search|where|what|which/.test(
          msgLower
        );
      ...
      if (route) {
        ...
        if (tripTitle && route.from && route.to) {
          tripTitle.textContent = `${route.from.city || ""} to ${
            route.to.city || ""
          }`;
        }
        // Show map button
      ...
      if (travelPlanText) {
        ...
        if (planEl) {
          ...
          let formatted = travelPlanText
            ...
            .replace(
              /### (.*?)(\n|$)/g,
              '<h3 style="color: #10b981; margin-top: 24px; margin-bottom: 12px; font-size: 18px;">$1</h3>'
            ) // H3
            .replace(
              /## (.*?)(\n|$)/g,
              '<h2 style="color: #10b981; margin-top: 28px; margin-bottom: 16px; font-size: 20px;">$1</h2>'
            ) // H2
            .replace(
              /# (.*?)(\n|$)/g,
              '<h1 style="color: #10b981; margin-top: 32px; margin-bottom: 20px; font-size: 24px;">$1</h1>'
            ) // H1
            .replace(/\n/g, "<br>"); // Line breaks
          ...
          searchResults.forEach((result, i) => {
            try {
              ...
              card.innerHTML = `
                ...
                ${
                  snippet
                    ? `<div class="search-result-snippet">${formatMessage(
                        snippet
                      )}</div>`
                    : ""
                }
                ${
                  displayLink
                    ? `<a href="${
                        link.startsWith("http") ? link : "https://" + link
                      }" target="_blank" rel="noopener noreferrer" class="search-result-link">${displayLink}</a>`
                    : ""
                }
              `;
    ...
    voiceBtn.addEventListener("click", () => {
      if (isRecording) {
        ...
        try {
          ...
          addMsgWithVoice(
            "Voice input is not available. Please use text input.",
            "bot"
          );
        }
      }
  ...
  const mapContainerEl = document.getElementById("mapContainer");
  if (mapContainerEl) {
    mapObserver.observe(mapContainerEl, {
      attributes: true,
    ...
    mutations.forEach((mutation) => {
      if (mutation.addedNodes.length > 0) {
        ...
        for (let node of mutation.addedNodes) {
          if (
            node.nodeType === 1 &&
            (node.classList.contains("msg") || node.querySelector(".msg"))
          ) {
            shouldScroll = true;
            break;
          }
  ...
  if (chatMessagesEl && chatMessagesEl.children.length === 0) {
    addMsg(
      "Hi there! I am Naveo AI agent. How can I help you plan your trip today? ✈️",
      "bot"
    );
  }
